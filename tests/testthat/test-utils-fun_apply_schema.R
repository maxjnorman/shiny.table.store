test_that("utils-fun_apply_schema-skip_keys_ignore", {
  ...
  make_obj <- purrr::partial(fun_apply_schema, data = data, schema = schema)
...
test_that("utils-fun_apply_schema-full_keys_ignore", {
...
test_that("utils-fun_apply_schema-valid_keys_ignore", {
  data <- tibble::tibble("x" = as.character(1:3), "y" = as.character(4:6))
  schema <- list("x" = as.character(9:1), "y" = as.character(9:1))
  ...
  expect_identical(fun_apply_schema(data, schema, keys_ignore = c("y")), outX)
  expect_identical(fun_apply_schema(data, schema, keys_ignore = c("x")), outY)
})
