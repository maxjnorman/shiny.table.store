...
test_that("utils-tbl_from_history-simple_combo", {
  A <- tibble::tibble(x = 1:3)
  B <- tibble::tibble(x = 2:4)
  C <- tibble::tibble(x = 1:4) # sorted out
  D <- tibble::tibble(x = c(2:4, 1)) # unsorted out
  # named lists should sort by name order
  expect_mapequal(C, tbl_from_history(list("A" = A, "B" = B)))
  expect_mapequal(C, tbl_from_history(list("B" = B, "A" = A)))
  # unnamed lists do not sort
  expect_mapequal(C, tbl_from_history(list(A, B)))
  expect_mapequal(D, tbl_from_history(list(B, A)))
})
